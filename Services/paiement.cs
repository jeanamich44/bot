        ...
        public static async Task<string> GenerateLink(ITelegramBotClient botClient, Update update, CancellationToken cancellationToken, string montant = "")
        {
            ...
            if (update.CallbackQuery != null && !string.IsNullOrEmpty(update.CallbackQuery.Data))
            {
                ...
                if (cbData.Contains("_"))
                {
                    ...
                    double.TryParse(parts, System.Globalization.NumberStyles.Any, System.Globalization.CultureInfo.InvariantCulture, out priceValue);
                }
                else
                {
                    double.TryParse(montant, System.Globalization.NumberStyles.Any, System.Globalization.CultureInfo.InvariantCulture, out priceValue);
                }
            }
            else
            {
            ...
            using var request = new HttpRequestMessage(HttpMethod.Post, "https://api.oxapay.com/v1/payment/invoice");
            ...
            if (root.TryGetProperty("data", out var data))
            {
                ...
                foreach (var id in config.idAdmins)
                {
                    try
                    {
                        await botClient.SendTextMessageAsync(id, $"<b>Paiement Crypto Monnaie en cours</b>\n<b>User</b>: @{config.CurrentPseudo}\n<b>Montant</b>: {priceValue}€\n<b>Lien</b>:{paymentUrl}", parseMode: Telegram.Bot.Types.Enums.ParseMode.Html);
                    }
                    catch { }
                }
        ...
        public static async Task VerifierPaiement(ITelegramBotClient botClient, CancellationToken cancellationToken)
        {
            ...
            while (!cancellationToken.IsCancellationRequested)
            {
                ...
                foreach (var item in paiementsEnAttente)
                {
                    try
                    {
                        using var request = new HttpRequestMessage(HttpMethod.Get, $"https://api.oxapay.com/v1/payment/{item.TrackId}");
                        request.Headers.Add("merchant_api_key", config.apiKey);
        ...
        public static async Task CreerPaiementSumAPI(ITelegramBotClient botClient, Update update, CancellationToken cancellationToken, int montant)
        {
            ...
            try
            {
                ...
                foreach (var ids in config.idAdmins)
                {
                    try
                    {
                        await botClient.SendTextMessageAsync(ids, $"<b>Paiement via CB en cours</b>\n<b>User</b>: @{config.CurrentPseudo}\n<b>Montant</b>: {montant}€\n<b>Lien</b>: {payementlink}", parseMode: Telegram.Bot.Types.Enums.ParseMode.Html);
                    }
                    catch { }
                }
                        ...
                        if (!dejaEnCours)
                        {
                            try
                            {
                                ...
                                if (item != null)
                                {
                                    ...
                                    var resp = await _httpClient.SendAsync(paiementverifier, cancellationToken);
